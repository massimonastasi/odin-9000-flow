...
if (!args.root || !args.file) {
  console.error('Usage: echo <plan> | node generate-phase3.mjs --root <nodeId> --file <fileKey> [--out path]');
  process.exit(1);
}
...
lines.push(` * Bindings: ${plan.length} | Nodes: ${Object.keys(byNode).length}`);
...
lines.push(`      if (k.endsWith(suffix) && k.includes(name.split('/')[0])) return v;`);
lines.push(`    }`);
...
for (const [nodeId, writes] of Object.entries(byNode)) {
  ...
  lines.push(`    if (!node) { report.failed += ${writes.length}; report.log.push({nodeId:"${nodeId}",error:"not found"}); }`);
  lines.push(`    else {`);
  ...
  for (const w of writes) {
    ...
    if (w.prop === 'fills' || w.prop === 'strokes') {
      ...
      lines.push(`        if (v) { bindPaint(node, "${w.prop}", v); report.applied++; report.log.push({nodeId:"${nodeId}",prop:"${w.prop}",status:"ok"}); }`);
      lines.push(`        else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"${w.prop}",error:"var not found: ${tokenName}"}); }`);
      lines.push(`      }`);
    } else if (w.prop === 'cornerRadius') {
      lines.push(`      { // cornerRadius → ${tokenName}`);
      lines.push(`        const v = findVar("${tokenName}");`);
      lines.push(`        if (v) {`);
      lines.push(`          for (const p of ['topLeftRadius','topRightRadius','bottomLeftRadius','bottomRightRadius']) node.setBoundVariable(p, v);`);
      lines.push(`          report.applied++; report.log.push({nodeId:"${nodeId}",prop:"cornerRadius",status:"ok"});`);
      lines.push(`        } else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"cornerRadius",error:"var not found: ${tokenName}"}); }`);
      lines.push(`      }`);
    } else if (w.prop === 'padding') {
      lines.push(`      { // padding (all 4) → ${tokenName}`);
      lines.push(`        const v = findVar("${tokenName}");`);
      lines.push(`        if (v) {`);
      lines.push(`          for (const p of ['paddingTop','paddingRight','paddingBottom','paddingLeft']) node.setBoundVariable(p, v);`);
      lines.push(`          report.applied++; report.log.push({nodeId:"${nodeId}",prop:"padding",status:"ok"});`);
      lines.push(`        } else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"padding",error:"var not found: ${tokenName}"}); }`);
      lines.push(`      }`);
    } else if (w.prop === 'paddingH') {
      lines.push(`      { // paddingH → ${tokenName}`);
      lines.push(`        const v = findVar("${tokenName}");`);
      lines.push(`        if (v) {`);
      lines.push(`          node.setBoundVariable('paddingLeft', v); node.setBoundVariable('paddingRight', v);`);
      lines.push(`          report.applied++; report.log.push({nodeId:"${nodeId}",prop:"paddingH",status:"ok"});`);
      lines.push(`        } else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"paddingH",error:"var not found: ${tokenName}"}); }`);
      lines.push(`      }`);
    } else if (w.prop === 'paddingV') {
      lines.push(`      { // paddingV → ${tokenName}`);
      lines.push(`        const v = findVar("${tokenName}");`);
      lines.push(`        if (v) {`);
      lines.push(`          node.setBoundVariable('paddingTop', v); node.setBoundVariable('paddingBottom', v);`);
      lines.push(`          report.applied++; report.log.push({nodeId:"${nodeId}",prop:"paddingV",status:"ok"});`);
      lines.push(`        } else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"paddingV",error:"var not found: ${tokenName}"}); }`);
      lines.push(`      }`);
    } else if (w.prop === 'strokeWeight') {
      lines.push(`      { // strokeWeight → ${tokenName}`);
      lines.push(`        const v = findVar("${tokenName}");`);
      lines.push(`        if (v) {`);
      lines.push(`          for (const p of ['strokeTopWeight','strokeBottomWeight','strokeLeftWeight','strokeRightWeight']) node.setBoundVariable(p, v);`);
      lines.push(`          report.applied++; report.log.push({nodeId:"${nodeId}",prop:"strokeWeight",status:"ok"});`);
      lines.push(`        } else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"strokeWeight",error:"var not found: ${tokenName}"}); }`);
      lines.push(`      }`);
      ...
      lines.push(`          report.applied++; report.log.push({nodeId:"${nodeId}",prop:"itemSpacing",status:"ok"});`);
      lines.push(`        } else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"itemSpacing",error:"var not found: ${tokenName}"}); }`);
      lines.push(`      }`);
      ...
      lines.push(`        if (s) { await node.setTextStyleIdAsync(s.id); report.applied++; report.log.push({nodeId:"${nodeId}",prop:"textStyle",status:"ok"}); }`);
      lines.push(`        else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"textStyle",error:"style not found: ${styleName}"}); }`);
      lines.push(`      }`);
    } else {
      // Generic scalar prop
      lines.push(`      { // ${w.prop} → ${tokenName}`);
      lines.push(`        const v = findVar("${tokenName}");`);
      lines.push(`        if (v) { node.setBoundVariable("${w.prop}", v); report.applied++; report.log.push({nodeId:"${nodeId}",prop:"${w.prop}",status:"ok"}); }`);
      lines.push(`        else { report.failed++; report.log.push({nodeId:"${nodeId}",prop:"${w.prop}",error:"var not found: ${tokenName}"}); }`);
      lines.push(`      }`);
    }
  }
...
lines.push(`  figma.closePlugin(JSON.stringify({ applied: report.applied, failed: report.failed }));`);
lines.push(`})();`);
...
if (args.out) {
  ...
  console.log(JSON.stringify({ ok: true, path: args.out, bindings: plan.length, nodes: Object.keys(byNode).length }));
} else {
  console.log(script);
}
