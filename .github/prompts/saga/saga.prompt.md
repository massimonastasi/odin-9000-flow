...
> **Never re-read your own output files.** Once you write a file (CSS, TSX, stories, MDX),
> do NOT read it back to verify — trust the write. Re-reading your own output wastes context
> tokens and causes multi-read loops (observed: 11 re-reads of a CSS file = 117s wasted).

> **Use the CSS skeleton when ODIN provides one.** If ODIN passes a `PRE_GENERATED_CSS` path
> in the dispatch, that file already contains the complete shadow-DOM CSS with all `--fds-*`
> vars and fallbacks (generated by `generate-component-css.mjs`). Read it ONCE as your CSS
> base. Only add rules for things the skeleton can't express (`:host([variant])` states,
> media queries, animations). Do NOT regenerate the CSS from scratch.

> **Read reference files exactly once.** Read `fds-bonus-bar/*.tsx` (or whatever reference
> component ODIN specifies) once for pattern matching, then generate. Do not re-read.
